...
# -------------------------------------------------------------------------
    ...
    # [풍자 후원] 특수활동비(비자금) 컨셉
    st.markdown("---")
    with st.expander("🤫 청와대 특수활동비(비자금) 집행하기"):
        st.warning("""
        **"이 돈은 국고에 기록되지 않으며, 전액 개발자의 커피값으로 투명하게(?) 횡령됩니다."**
        \n(※ 대가성 없는 순수한 마음의 사과박스입니다.)
        """)
        # ▼ 아래 주소를 본인 토스 아이디로 바꾸세요
        st.link_button("🍎 사과박스(후원금) 은밀하게 건네기", "https://toss.me/본인아이디", use_container_width=True)
