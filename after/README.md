...
성형 후 예상 얼굴을 AI로 만들고, 그 얼굴로 찍은 인스타용 인생샷을 생성해 공유하는 웹앱.
...
│  │  └─ api/               session, upload, generate/{face,photos}, generations/[id], share, delete, waitlist, cron/cleanup
│  ├─ components/           슬라이더, 로딩 문장, 페이월 모달, 삭제 버튼
│  ├─ lib/
...
3. 얼굴 생성: 분당 제한 → 크레딧 차감 → 20분짜리 서명 URL 로 모델에 전달 → 큐에 제출. 브라우저가 `GET /api/generations/[id]` 를 2.5초마다 폴링하고, 완료되면 그 요청 안에서 결과를 내려받아 **워터마크를 굽고 public 버킷**에 저장한다. 워터마크 없는 사본은 private 에만 둔다(인생샷 레퍼런스·공유 카드 합성용).
4. 인생샷: 레퍼런스 순서 = 성형 후 얼굴(깨끗한 사본) → 원본 정면 → 원본 측면. 4장 한 세트.
5. 공유: OG 1200×630(애프터 얼굴 + 인생샷 4장) 은 public, 스토리 1080×1920 5장(1장은 비포/애프터라 원본 포함) 은 private 에 저장하고 본인만 서명 URL 로 내려받는다. 공유 페이지 `/s/[id]` 는 public 파일만 참조한다.
6. 삭제: "내 데이터 즉시 삭제" 버튼 → 원본·생성물·공유 전부 삭제. 크론은 30분마다 만료분 삭제.
...
- [ ] 실제 결과를 보고 `prompts/surgery.ts` 의 강도별 문장과 `FACE_GUIDANCE_SCALE`(기본 3.5) 튜닝. Kontext 가 정체성을 무너뜨리면 guidance 를 낮추고, 변화가 약하면 문장을 세게.
...
- [ ] 간단한 이벤트 로깅(시작, 업로드, 얼굴 완료, 인생샷 완료, 공유 클릭)으로 퍼널 측정.
...
| 기준 | 상태 |
|---|---|
| 셀카 → 성형 후 얼굴 | 코드 완성, 목으로 전 구간 확인. 실제 모델은 키 투입 후 확인 필요 |
| 인생샷 4장 | 동일 |
